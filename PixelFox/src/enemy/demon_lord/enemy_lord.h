...
{
    class DemonLord final : public IEnemy
    {
    public:
        DemonLord() = default;
        ...
        void SetInvisible() override
        {
            ...
            if (m_pFont)
            {
                pixel_engine::PERenderQueue::Instance().RemoveFont(m_pFont.get());
            }
        }
    ...
    private:
        ...
        std::string m_szBaseFile{ "assets/sprites/Enemy/Demon/" };

        //~ Internals
        std::unique_ptr<pixel_engine::QuadObject>      m_pBody{ nullptr };
        std::unique_ptr<pixel_engine::PEFont>      m_pFont{ nullptr };
        ...
        bool      m_bShowingHP{ false };
        float     m_nHealth{ 9000.0f };
        float     m_nMaxHealth{ 9000.0f };
        float     m_nMoveSpeedUPS{ 1.5f };
        float     m_nDamage{ 300.f };
        float     m_nKnockBack{ 5.f };
        FVector2D m_scale{ 10.0f, 10.0f };
