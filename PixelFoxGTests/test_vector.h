...
//        EXPECT_EQ((std::vector<int>{v.begin(), v.end()}), (std::vector<int>{1, 1, 2, 3, 2, 3}));
//    }
//
//    // ---------- 9) Exception safety ----------
//    TEST(Vector_ExceptionSafety, StrongGuaranteeOnInsertReallocCopyThrow) {
//        ThrowOn::mask = ThrowOn::ThrowOnCopy;
//        FOX_VECTOR<ThrowOn> v; v.push_back(ThrowOn{ 1 }); v.push_back(ThrowOn{ 2 });
//        // Force reallocation scenario
//        v.reserve(2);
//        EXPECT_THROW({ v.insert(v.begin() + 1, ThrowOn{3}); }, std::runtime_error);
//        // After throw, vector should remain unchanged
//        ASSERT_EQ(v.size(), 2u);
//        EXPECT_EQ(v[0].x, 1); EXPECT_EQ(v[1].x, 2);
...
//    TEST(DISABLED_Vector_Perf, PushBackMillionInts) {
